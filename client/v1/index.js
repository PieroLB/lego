...
// 2. Log the variable
const legoSetWithHighestReductionDealabs =
  "https://www.dealabs.com/bons-plans/lego-icons-10332-la-place-de-la-ville-medievale-3273747";
const legoSetWithHighestReductionAvenueDeLabrique =
  "https://www.avenuedelabrique.com/go/bp/805";
console.log(legoSetWithHighestReductionDealabs);
console.log(legoSetWithHighestReductionAvenueDeLabrique);
...
// 3. Log the variable
const sortList = (list, value, order = -1) => {
  return [...list].sort((a, b) => order * (a[value] - b[value]));
};
const sortDealsByPriceAsc = sortList(deals, "price", 1);
console.table(sortDealsByPriceAsc);

// 🎯 TODO 5: Sort by date
// 1. Create a function to sort the deals by date
// 2. Create a variable and assign it the list of deals by date from recent to old
// 3. Log the variable
const sortListByDate = (list, value, order = -1) => {
  return [...list].sort(
    (a, b) => order * (new Date(a[value]) - new Date(b[value])),
  );
};
const dealsSortByDateDesc = sortListByDate(deals, "published", -1);
console.table(dealsSortByDateDesc);
...
const averageDiscount =
  deals.reduce((sum, deal) => sum + deal.discount, 0) / numberOfDeals;
...
for (const deal of deals) {
  if (!communities[deal.community]) communities[deal.community] = [];
...
// 2. Log the sort
for (const [communityName, communityDeals] of Object.entries(communities)) {
  const communityDealsSorted = sortList(communityDeals, "price", -1);
...
// 2. Log the item
const specificItem = VINTED.find(
  (p) => p.uuid === "f2c5377c-84f9-571d-8712-98902dcbb913",
);
console.log(specificItem || "item not found sry :(");
...
// 2. Log the value
const highestItemPrice = Math.max(...VINTED.map((p) => parseFloat(p.price)));
const profitability = highestItemPrice - deal.price;
